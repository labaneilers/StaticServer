...
namespace PngLib
{
    internal abstract class PngWriterInternal
    {
        ...
        internal PngWriterInternal(Bitmap bitmap, Stream outputStream, ColorType colorType, PngFilterType filterType)
        {
            _bitmap = bitmap;
            ...
            switch (filterType)
            {
                ...
                case PngFilterType.Sub:
                    break;
                default:
                    throw new Exception(String.Format("Png24Writer: Unsupported filter {0}", filterType));
            }
        }
        ...
        private void WriteBitmapData()
        {
            ...
            var length = (uint)memoryStream.Length;
        ...
        protected static byte[] CHUNK_TYPE_PALETTE = Encoding.ASCII.GetBytes("PLTE");
        protected virtual void WritePalette() { }

        protected static byte[] CHUNK_TYPE_TRANSPARENCY = Encoding.ASCII.GetBytes("tRNS");        
        ...
        private static readonly byte[] CHUNK_TYPE_FOOTER = Encoding.ASCII.GetBytes("IEND");
        private static readonly byte[] FOOTER_DATA = { };
        private void WriteFooter()
        {
            WriteChunk(CHUNK_TYPE_FOOTER, FOOTER_DATA);
        }

        private static readonly byte[] CHUNK_TYPE_HEADER = Encoding.ASCII.GetBytes("IHDR");
        private void WriteHeader()
        {
            var header = new MemoryStream(13);
        ...
        private static readonly byte[] CHUNK_TYPE_DATA = Encoding.ASCII.GetBytes("IDAT");
